// BullrunIQ — shared auth helpers (used by auth.js, sync.js, generate.js).
// Not a handler — prefixed with _ so Netlify does not deploy it as a function.
...
function verifyToken(tok) {
  try {
    ...
    if (!email || !exp || isNaN(exp) || Date.now() > exp) return null;
    return email;
  } catch (e) { return null; }
}
