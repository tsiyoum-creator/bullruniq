...
const { secretKey, signToken, planFor, json } = require("./_shared");
...
exports.handler = async function (event) {
  ...
  if (!secretKey()) return json(500, { error: "Auth not configured on server." }, CORS);

  let p = {};
  try { p = JSON.parse(event.body || "{}"); } catch (e) {}
  const action = p.action;
  const email = String(p.email || "").trim().toLowerCase();
  if (!email || email.indexOf("@") < 1 || email.length > 200) return json(400, { error: "Enter a valid email." }, CORS);

  const blobs = require("@netlify/blobs");
  try { blobs.connectLambda(event); } catch (e) {}
  const getStore = blobs.getStore;
  let store;
  try { store = getStore("auth"); } catch (e) { return json(500, { error: "Storage unavailable — try again shortly." }, CORS); }

  try {

  if (action === "request") {
    const RESEND = process.env.RESEND_API_KEY;
    if (!RESEND) return json(500, { error: "Email login isn't enabled yet (server needs RESEND_API_KEY)." }, CORS);
    const prev = await store.get(email, { type: "json" });
    if (prev && prev.sent >= 3 && Date.now() < prev.exp) return json(429, { error: "Too many codes requested — try again in a few minutes." }, CORS);
    ...
    if (!r.ok) return json(502, { error: "Couldn't send the email — try again." }, CORS);
  ...
  if (action === "verify") {
    ...
    if (!rec || Date.now() > rec.exp) return json(400, { error: "Code expired — request a new one." }, CORS);
    if (rec.tries >= 5) return json(429, { error: "Too many attempts — request a new code." }, CORS);
    if (sha(email + ":" + code) !== rec.hash) {
      rec.tries = (rec.tries || 0) + 1;
      await store.setJSON(email, rec);
      return json(400, { error: "Wrong code — check the email and try again." }, CORS);
    }
    ...
    return json(200, { token: signToken(email, 30), email: email, plan: plan }, CORS);
  }
  ...
  return json(400, { error: "Unknown action" }, CORS);
    ...
    return json(500, { error: "Login service hiccup — try again shortly." }, CORS);
  }
};
