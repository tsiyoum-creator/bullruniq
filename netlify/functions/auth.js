...
// Secret: AUTH_SECRET env var, else derived from ANTHROPIC_API_KEY.

const crypto = require("crypto");
const { secretKey, signToken, planFor, esc, json, isValidEmail } = require("./_lib");
...
exports.handler = async function (event) {
  ...
  if (!secretKey()) return jsonCors(500, { error: "Auth not configured on server." });

  let p = {};
  try { p = JSON.parse(event.body || "{}"); } catch (e) {}
  const action = p.action;
  const email = String(p.email || "").trim().toLowerCase();
  if (!isValidEmail(email)) return jsonCors(400, { error: "Enter a valid email." });

  const blobs = require("@netlify/blobs");
  try { blobs.connectLambda(event); } catch (e) {}
  const getStore = blobs.getStore;
  let store;
  try { store = getStore("auth"); } catch (e) { return jsonCors(500, { error: "Storage unavailable — try again shortly." }); }
    ...
    const r = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: "<!doctype html><html><head><meta charset='utf-8'></head><body style='margin:0;background:#050505;padding:40px 24px;font-family:-apple-system,Segoe UI,sans-serif;text-align:center'>"
          ...
          + "<div style='font-family:Georgia,serif;font-size:42px;letter-spacing:10px;color:#c9a84c'>" + safeCode + "</div>"
          + "<div style='color:#5c574e;font-size:11px;margin-top:28px'>If you didn't request this, you can ignore this email.</div>"
          + "</body></html>",
      }),
    });
    if (!r.ok) return jsonCors(502, { error: "Couldn't send the email — try again." });
    return jsonCors(200, { ok: true });
  }

  if (action === "verify") {
    const code = String(p.code || "").trim();
    if (!code || !/^\d{6}$/.test(code)) return jsonCors(400, { error: "Invalid code format." });
  ...
  return jsonCors(400, { error: "Unknown action" });
    ...
    return jsonCors(500, { error: "Login service hiccup — try again shortly." });
  }
};
