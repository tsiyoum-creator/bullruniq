...
const { CORS_POST, json, secretKey, signToken, sha256, planFor, esc, isValidEmail } = require("./_shared");
...
exports.handler = async function (event) {
  ...
  if (action === "request") {
    ...
    const html = "<!doctype html><html><head><meta charset='utf-8'></head><body style='margin:0;background:#050505;padding:40px 24px;font-family:-apple-system,Segoe UI,sans-serif;text-align:center'>"
      ...
      + "<div style='font-family:Georgia,serif;font-size:42px;letter-spacing:10px;color:#c9a84c'>" + esc(code) + "</div>"
    ...
    const r = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from, to: email, subject: code + " is your BullrunIQ login code", html }),
    });
