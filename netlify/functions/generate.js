...
async function dailyOk(key, cap) {
  try {
    ...
    // Blobs unavailable — fail open for authenticated users, fail closed for anon
    console.log("[generate] dailyOk blobs error:", e.message);
    return key.startsWith("user:") ? true : false;
  }
}
...
exports.handler = async function (event) {
  ...
  if (system) body.system = String(system).slice(0, MAX_SYSTEM_LENGTH);
