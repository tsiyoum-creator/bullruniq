...
const { verifyToken, planFor } = require("./_shared");
...
exports.handler = async function (event) {
  ...
  if (event.httpMethod === "POST") {
    ...
    let serialized;
    try { serialized = JSON.stringify(p.data); } catch (e) { return json(400, { error: "Data is not serializable" }); }
    await store.setJSON(email, { data: JSON.parse(serialized), updatedAt: new Date().toISOString() });
