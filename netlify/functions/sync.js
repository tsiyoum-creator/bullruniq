...
exports.handler = async function (event) {
  ...
  if (event.httpMethod === "POST") {
    // Netlify may base64-encode the body; decode it first before checking size
    const rawBody = event.isBase64Encoded
      ? Buffer.from(event.body || "", "base64").toString("utf8")
      : (event.body || "");
    if (rawBody.length > MAX_BYTES) return json(413, { error: "State too large." });
    let p = {};
    try { p = JSON.parse(rawBody); } catch (e) { return json(400, { error: "Bad JSON" }); }
