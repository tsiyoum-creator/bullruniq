...
// serverAlerted flags with ~5% hysteresis prevent repeat emails.
...
function emailFooter(email, reason) {
  return "<div style='border-top:1px solid #1a1a1a;margin-top:32px;padding-top:16px;font-size:11px;color:#5c574e;line-height:1.6;max-width:420px;margin-left:auto;margin-right:auto'>Educational alert, not financial advice. You get these because you set " + reason + " in BullrunIQ.<br><a href='https://bullruniq.com/api/unsubscribe?email=" + encodeURIComponent(email) + "' style='color:#8a8278'>Unsubscribe from all emails</a></div>";
}
...
function buyAlertHtml(w, price, email) {
  ...
  return emailShell(
    ...
    + emailFooter(email, "a price target")
  );
}
...
function sellAlertHtml(w, price, gainPct, email) {
  ...
  return emailShell(
    ...
    + emailFooter(email, "a sell target")
  );
}
...
function tpAlertHtml(h, price, email) {
  ...
  return emailShell(
    ...
    + emailFooter(email, "a take-profit")
  );
}

async function sendAlert(RESEND, FROM, to, subject, html) {
  ...
  if (!r.ok) {
    ...
    console.log("[alerts] Resend error for " + to + ": " + msg);
  }
  return r.ok;
}
...
exports.handler = async function (event) {
    ...
    try {
      ...
      if (hasWl || hasHold) {
        ...
        wlist.forEach(function (w) {
          if (w && (w.targetPrice || w.sellTarget)) ids.add(cgId(w.ticker));
        });
        hold.forEach(function (h) {
          if (h && (h.stop || h.tp)) ids.add(cgId(h.ticker));
        });
      }
    } catch (e) {}
  }
  ...
  for (const email of Object.keys(recs)) {
    ...
    // Portfolio guard: stop-loss / take-profit on holdings
    const hold = rec.data.port && Array.isArray(rec.data.port.crypto) ? rec.data.port.crypto : [];
    for (const h of hold) {
      ...
      if (h.stop && p <= h.stop && !h.serverStopAlerted && sent < MAX_EMAILS_PER_RUN) {
        const ok = await sendAlert(RESEND, FROM, email, "⛔ " + h.ticker + " fell below your stop-loss — now " + fp(p), stopAlertHtml(h, p, email));
      ...
      if (h.tp && p >= h.tp && !h.serverTpAlerted && sent < MAX_EMAILS_PER_RUN) {
        const ok = await sendAlert(RESEND, FROM, email, "🎯 " + h.ticker + " hit your take-profit — now " + fp(p), tpAlertHtml(h, p, email));
    ...
    for (const w of wlist) {
      ...
      if (w.targetPrice) {
        ...
        if (dist < 2 && p <= w.targetPrice * 1.02 && !w.serverAlerted && sent < MAX_EMAILS_PER_RUN) {
          const ok = await sendAlert(RESEND, FROM, email, "🎯 " + w.ticker + " hit your buy zone — now " + fp(p), buyAlertHtml(w, p, email));
      ...
      if (w.sellTarget && p >= w.sellTarget && !w.serverSellAlerted && sent < MAX_EMAILS_PER_RUN) {
        ...
        const ok = await sendAlert(RESEND, FROM, email, "💰 " + w.ticker + " hit your sell target — now " + fp(p), sellAlertHtml(w, p, gainPct, email));
