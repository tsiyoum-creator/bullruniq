...
// Alert flags live in rec._alerts (not rec.data) so user syncs don't reset them.
...
function unsubUrl(email, secret) {
  const tok = secret ? "&t=" + makeUnsubToken(email, secret) : "";
  return "https://bullruniq.com/api/unsubscribe?email=" + encodeURIComponent(email) + tok;
}

function buyAlertHtml(w, price, email, secret) {
...
function stopAlertHtml(h, price, email, secret) {
...
function tpAlertHtml(h, price, email, secret) {
  const name = esc(h.name || h.ticker);
  const ticker = esc(h.ticker);
...
exports.handler = async function (event) {
  ...
  const UNSUB_SECRET = process.env.AUTH_SECRET || process.env.ANTHROPIC_API_KEY;
  ...
  for (const email of Object.keys(recs)) {
    ...
    // Alert flags live in rec._alerts (not rec.data) so user syncs from the
    // client don't overwrite them. sync.js preserves _alerts across writes.
    const alerts = rec._alerts || {};
    let changed = false;

    function getFlag(key) { return !!alerts[key]; }
    function setFlag(key, val) { if (!!alerts[key] !== !!val) { alerts[key] = val || undefined; changed = true; } }
    ...
    for (const h of hold) {
      ...
      if (!p) continue;
      const stopKey = "h:" + h.ticker + ":stop";
      const tpKey   = "h:" + h.ticker + ":tp";

      if (h.stop && p <= h.stop && !getFlag(stopKey) && sent < MAX_EMAILS_PER_RUN) {
        try {
          const r = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              ...
              html: stopAlertHtml(h, p, email, UNSUB_SECRET),
            }),
          });
          if (r.ok) { sent++; setFlag(stopKey, true); console.log("[alerts] stop " + email + " " + h.ticker + " @ " + p); }
        } catch (e) {}
      } else if (h.stop && p >= h.stop * 1.05 && getFlag(stopKey)) {
        setFlag(stopKey, false); // re-arm once price recovers 5% above the stop
      }

      if (h.tp && p >= h.tp && !getFlag(tpKey) && sent < MAX_EMAILS_PER_RUN) {
        try {
          const r = await fetch("https://api.resend.com/emails", {
            method: "POST",
            headers: { Authorization: "Bearer " + RESEND, "Content-Type": "application/json" },
            body: JSON.stringify({
              ...
              html: tpAlertHtml(h, p, email, UNSUB_SECRET),
            }),
          });
          if (r.ok) { sent++; setFlag(tpKey, true); console.log("[alerts] tp " + email + " " + h.ticker + " @ " + p); }
        } catch (e) {}
      } else if (h.tp && p < h.tp * 0.95 && getFlag(tpKey)) {
        setFlag(tpKey, false); // re-arm once price retraces 5% below the target
      }
    }
    ...
    for (const w of rec.data.wl || []) {
      ...
      const buyKey  = "w:" + w.ticker + ":buy";
      const sellKey = "w:" + w.ticker + ":sell";

      // BUY alert: price within 2% below the buy target (approaching from above)
      if (w.targetPrice) {
        const dist = Math.abs((w.targetPrice - p) / p * 100);
        if (dist < 2 && p <= w.targetPrice * 1.02 && !getFlag(buyKey) && sent < MAX_EMAILS_PER_RUN) {
          try {
            const r = await fetch("https://api.resend.com/emails", {
              method: "POST",
              headers: { Authorization: "Bearer " + RESEND, "Content-Type": "application/json" },
              body: JSON.stringify({
                ...
                html: buyAlertHtml(w, p, email, UNSUB_SECRET),
              }),
            });
            if (r.ok) { sent++; setFlag(buyKey, true); console.log("[alerts] buy " + email + " " + w.ticker + " @ " + p); }
          } catch (e) {}
        } else if (dist >= 5 && getFlag(buyKey)) {
          setFlag(buyKey, false); // re-arm once price moves away
        }
      }

      // SELL alert: price at or above the sell target (profit-taking signal)
      if (w.sellTarget && p >= w.sellTarget && !getFlag(sellKey) && sent < MAX_EMAILS_PER_RUN) {
        ...
        try {
          const r = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              ...
              html: sellAlertHtml(w, p, gainPct, email, UNSUB_SECRET),
            }),
          });
          if (r.ok) { sent++; setFlag(sellKey, true); console.log("[alerts] sell " + email + " " + w.ticker + " @ " + p); }
        } catch (e) {}
      } else if (w.sellTarget && p < w.sellTarget * 0.95 && getFlag(sellKey)) {
        setFlag(sellKey, false); // re-arm once price retraces 5%
      }
    }
    if (changed) { try { rec._alerts = alerts; await store.setJSON(email, rec); } catch (e) {} }
  }
