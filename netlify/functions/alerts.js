...
const MAX_PARALLEL_USERS = 10; // cap concurrent Blobs reads
...
const CGMAP = {
  ...
  OP:"optimism", ARB:"arbitrum", SUI:"sui", INJ:"injective-protocol", PEPE:"pepe",
  WIF:"dogwifcoin", TON:"the-open-network", XLM:"stellar", HBAR:"hedera-hashgraph",
  QNT:"quant-network", AERO:"aerodrome-finance", ALGO:"algorand", VET:"vechain",
  ...
  POL:"polygon-ecosystem-token", W:"wormhole", BEAM:"beam-2", ORDI:"ordinals",
  SATS:"1000sats-ordinals", MEME:"memecoin", TIA:"celestia", PYUSD:"paypal-usd",
  ANKR:"ankr", CFX:"conflux-token", ROSE:"oasis-network", ZIL:"zilliqa",
  THETA:"theta-token", GRT:"the-graph", MASK:"mask-network", ENS:"ethereum-name-service",
  1INCH:"1inch", SNX:"synthetix-network-token", CRV:"curve-dao-token",
  AAVE:"aave", MKR:"maker", COMP:"compound-governance-token",
  DYDX:"dydx", GMX:"gmx", PENDLE:"pendle",
  WLD:"worldcoin-wld", TNSR:"tensor", MYRO:"myro", POPCAT:"popcat",
};
...
function unsubUrl(email) {
  return "https://bullruniq.com/api/unsubscribe?email=" + encodeURIComponent(email) + "&tok=" + unsubToken(email);
}
...
function emailFooter(email) {
  return "<div style='border-top:1px solid #1a1a1a;margin-top:32px;padding-top:16px;font-size:11px;color:#5c574e;line-height:1.6;max-width:420px;margin-left:auto;margin-right:auto'>"
    + "Educational alert, not financial advice. You get these because you set a price target in BullrunIQ.<br>"
    + "<a href='" + unsubUrl(email) + "' style='color:#8a8278'>Unsubscribe from all emails</a></div>";
}
...
async function loadUserBatch(store, emails) {
...
exports.handler = async function (event) {
  ...
  // Load user records in parallel batches
  const recs = {};
  const ids = new Set();
  for (let i = 0; i < users.length; i += MAX_PARALLEL_USERS) {
    const batch = await loadUserBatch(store, users.slice(i, i + MAX_PARALLEL_USERS));
    for (const { email, rec } of batch) {
      if (!rec || !rec.data) continue;
      const wlist = Array.isArray(rec.data.wl) ? rec.data.wl : [];
  ...
  for (const email of Object.keys(recs)) {
    ...
    for (const h of hold) {
      ...
      if (h.stop && p <= h.stop && !h.serverStopAlerted && sent < MAX_EMAILS_PER_RUN) {
        try {
          if (await sendEmail(RESEND, email, "⛔ " + h.ticker + " fell below your stop-loss — now " + fp(p), stopAlertHtml(h, p, email))) {
      ...
      if (h.tp && p >= h.tp && !h.serverTpAlerted && sent < MAX_EMAILS_PER_RUN) {
        try {
          if (await sendEmail(RESEND, email, "🎯 " + h.ticker + " hit your take-profit — now " + fp(p), tpAlertHtml(h, p, email))) {
    ...
    for (const w of (rec.data.wl || [])) {
      ...
      if (w.targetPrice) {
        ...
        if (dist < 2 && p <= w.targetPrice * 1.02 && !w.serverAlerted && sent < MAX_EMAILS_PER_RUN) {
          try {
            if (await sendEmail(RESEND, email, "🎯 " + w.ticker + " hit your buy zone — now " + fp(p), buyAlertHtml(w, p, email))) {
      ...
      if (w.sellTarget && p >= w.sellTarget && !w.serverSellAlerted && sent < MAX_EMAILS_PER_RUN) {
        ...
        try {
          if (await sendEmail(RESEND, email, "💰 " + w.ticker + " hit your sell target — now " + fp(p), sellAlertHtml(w, p, gainPct, email))) {
